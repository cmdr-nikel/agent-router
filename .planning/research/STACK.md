...
**Domain:** Python library — trajectory-monitoring router bridging DSPy compile-time optimization and RouteLLM runtime routing
**Researched:** 2026-06-18
**Confidence:** HIGH (all critical items verified against installed source, PyPI, and ctx7 docs)
...
uv pip install "routellm[serve,eval]==0.2.0"

# Install embedding library (no torch pulled in)
uv pip install "fastembed==0.8.0"

# Install dev/test tools
uv pip install "pytest==9.1.0" "pytest-asyncio==1.4.0" "pytest-mock"

# Install the library itself in editable mode (once pyproject.toml exists)
uv pip install -e ".[dev]"
```

**Note:** `dspy==3.2.1`, `litellm==1.83.7`, `openai==2.30.0`, `numpy==2.4.4`, `pydantic==2.12.5` are already installed — no reinstall needed.
...
[tool.hatch.version]
path = "src/agent_router/__init__.py"
...
**If running validation benchmarks (synthetic loop bench / GSM8K):**
...
**If deploying RouteLLM server for development / integration testing:**
...
**If RouteLLM server is unavailable (unit tests / CI with no server):**
...
**If fastembed model first-load is too slow in CI:**
...
*Stack research for: agent-router (DSPy trajectory-monitoring router with RouteLLM)*
*Researched: 2026-06-18*
